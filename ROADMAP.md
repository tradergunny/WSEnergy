...
- **2026-06-26 (calc fixes)** — Estimator retuned for **exact PEA parity**, reverse-engineered from four benchmark runs (now pinned in `engine.test.ts`; all reproduce size/price/savings/payback/offset%/CO₂/fuel/trees). Coefficients calibrated to PEA's linear model: ฿480/kWp·mo, 498.8 kg CO₂/kWp·yr, ~220 L fuel oil/kWp·yr, ~8.05 trees/kWp (10-yr). **Water metric → fuel oil** (PEA's display); **CO₂ factor 0.475 → 0.3637** to match PEA (overrides our TGO sourcing). **Segment input dropped** — phase-only like PEA (1-phase ≤5 kW, 3-phase ≤20 kW); RFQ hand-off no longer carries `segment` (project type inferred from kWp, company optional for all estimator leads). **Package ladder reseeded** to PEA tiers — 3 confirmed prices + extrapolated tiers flagged as estimates (team overrides real WS prices in Studio). **Verdict relaxed to lean-to-recommend** (sales tool): always "Recommended" when a system fits + saves money (payback bands only pick sub-copy); "Not yet" survives only when nothing can be offered (roof can't hold the smallest tier, or no daytime load to offset). Supersedes the original 3-tier gate. Verified: 18 unit tests + live Sanity→engine E2E + browser. **Step 8 (map) still deferred**; next priority is UI/UX "easier reach".
