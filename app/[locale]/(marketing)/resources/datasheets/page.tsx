...
export async function generateMetadata({ params }: PageProps<"/[locale]/resources/datasheets">): Promise<Metadata> {
  const { locale } = await params;
  const dict = await getDictionary(locale);
  const t = dict.resourcePages.datasheets;
  return { title: t.headline, description: t.subhead, alternates: alternates("/resources/datasheets") };
}

export default async function DatasheetsPage({
  params,
}: PageProps<"/[locale]/resources/datasheets">) {
  const { locale } = await params;
  ...
  const docs = await sanityClient.fetch<DocRow[]>(docsByTypeQuery, {
    docType: "datasheet",
  });
