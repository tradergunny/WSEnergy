...
export async function generateMetadata({ params }: PageProps<"/[locale]/resources/standards-compliance">): Promise<Metadata> {
  const { locale } = await params;
  const dict = await getDictionary(locale);
  const t = dict.resourcePages.standards;
  return { title: t.headline, description: t.subhead, alternates: alternates("/resources/standards-compliance") };
}

export default async function StandardsCompliancePage({
  params,
}: PageProps<"/[locale]/resources/standards-compliance">) {
  const { locale } = await params;
  ...
  const [authorizations, certifications] = await Promise.all([
    sanityClient.fetch<DocRow[]>(docsByTypeQuery, { docType: "authorization" }),
    sanityClient.fetch<DocRow[]>(docsByTypeQuery, { docType: "certification" }),
  ]);

  const allDocs = [...authorizations, ...certifications];
