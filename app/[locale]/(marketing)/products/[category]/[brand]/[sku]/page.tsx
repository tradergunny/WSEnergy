...
export default async function ProductDetailPage({
  params,
}: PageProps<"/[locale]/products/[category]/[brand]/[sku]">) {
  ...
  const isSafety = isSafetyCategorySlug(
    product.category?.slug,
    product.category?.parentSlug,
  );
  const catHref = categoryHref(locale, {
    ...
    <>
      ...
      <Container as="nav" aria-label="Breadcrumb" className="pt-6">
        <ol className="text-caption text-graphite-600 flex flex-wrap items-center gap-1">
          ...
          <li>
            <Link
              href={withLocale(locale, isSafety ? "/safety" : "/products")}
              className="hover:text-brand-600"
            >
              {isSafety ? t.breadcrumbSafety : t.breadcrumbProducts}
            </Link>
          </li>
          <IconChevronRight size={12} stroke={1.5} aria-hidden />
          <li>
            <Link href={catHref} className="hover:text-brand-600">
              {categoryTitle}
            </Link>
          </li>
          <IconChevronRight size={12} stroke={1.5} aria-hidden />
          ...
          <ul className="flex gap-6 overflow-x-auto">
            {TABS.map((tab) => (
              <li key={tab}>
                <a
                  href={`#${tab}`}
                  className="text-body text-graphite-600 hover:text-brand-600 inline-block border-b-2 border-transparent px-1 py-3 font-medium"
                >
                  {t.tabs[tab]}
                </a>
              </li>
